...
import { Component, h, ref, nextTick, reactive } from 'vue'
import { NButton, NIcon, NSpace, useDialog, useMessage } from 'naive-ui'
import type { DataTableColumns, DropdownOption } from 'naive-ui'
import useStore from '@/store'
import { Add, Delete } from '@/components'
...
type Model = (typeof authcodeStore.data)[0]
const message = useMessage()
const dialog = useDialog()
const { authcodeStore, userStore } = useStore('manage')
...
/** 全局配置 */
// const value = ref(userStore.receiverStatus)
const status = [
  { label: '完全开放', value: 0 },
  { label: '启用', value: 1 },
  { label: '禁用', value: 2 }
    ...
    props: {
      onClick: () => {
        dialog.create({
          ...
          negativeText: '取消',
          onPositiveClick: () => {
            targetRow.value && authcodeStore.delete(targetRow.value.id)
          },
          onNegativeClick: () => {
            message.error('取消')
          }
      ...
      <div class="group">
        <n-flex>
          ...
          <n-popselect v-model:value="userStore.receiverConfig.status" :options="status" @update:value="handleStatusUpdate">
            <n-button secondary>
              {{ status[userStore.receiverConfig.status].label }}
            </n-button>
          </n-popselect>
          <!-- 显示筛选 -->
          <n-button secondary @click="columnSelect = !columnSelect">
             列可见
          </n-button>
        </n-flex>
      </div>
